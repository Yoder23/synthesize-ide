...
use command_guard::{classify, personal_terminal_policy, CommandPolicy, CommandRequest as GuardCommandRequest, CommandRisk};
use patch_engine::{
...
const FIXTURE_SOURCE: &str = "export function refreshToken() {\n  throw new Error(\"not implemented\");\n}\n";
...
fn open_repo_mock() -> Result<RepoOpenResult, String> {
    ...
    fs::write(repo.join("tests").join("auth").join("refresh.test.ts"), "import { refreshToken } from '../../src/auth/refresh';\n").map_err(|e| e.to_string())?;
...
fn open_repo_path(repo_root: String) -> Result<RepoOpenResult, String> {
    ...
    let first = files.iter().find(|f| !f.denied && f.kind == "file" && is_text_like(&f.path)).map(|f| f.path.clone()).unwrap_or_else(|| "README.md".into());
    repo_result(repo, &first)
}

fn repo_result(repo: PathBuf, file_path: &str) -> Result<RepoOpenResult, String> {
    let content = read_guarded_file(repo.to_string_lossy().to_string(), file_path.to_string()).unwrap_or_else(|_| "".into());
    let files = list_files_internal(&repo, 800)?;
    Ok(RepoOpenResult {
...
#[tauri::command]
fn clear_local_session_data(req: ClearLocalSessionDataRequest) -> Result<ClearLocalSessionDataResult, String> {
    let repo_root = PathBuf::from(&req.repo_root);
    let conn = init_audit(&repo_root, &req.session_id).map_err(|e| e.to_string())?;
    let cleared_runtime_requests = conn.execute("DELETE FROM runtime_requests WHERE session_id = ?1", params![&req.session_id]).map_err(|e| e.to_string())?;
    let cleared_context_bundles = conn.execute("DELETE FROM context_bundles WHERE session_id = ?1", params![&req.session_id]).map_err(|e| e.to_string())?;
    let cleared_audit_events = conn.execute("DELETE FROM audit_events WHERE session_id = ?1", params![&req.session_id]).map_err(|e| e.to_string())?;
    let cleared_endpoint_approvals = if req.clear_endpoint_approvals {
        conn.execute("DELETE FROM endpoint_approvals", []).map_err(|e| e.to_string())?
    } else {
        0
    };
    Ok(ClearLocalSessionDataResult {
        ...
        message: "Cleared local session context/runtime/audit data. Patch lifecycle records and checkpoints were preserved.".into(),
    })
}
...
fn build_context_bundle(req: ContextBundleRequest) -> Result<ContextBundleView, String> {
    ...
    let selected_path = guard.resolve_for_existing_path(&req.selected_file_path).map_err(|e| e.to_string())?;
    let selected_content = fs::read_to_string(&selected_path).map_err(|e| format!("failed to read selected file through RepoGuard: {}", e))?;
    ...
    let tree = files.iter()
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let package_scripts = read_package_scripts_excerpt(&repo_root).unwrap_or_else(|| "package scripts unavailable or denied by RepoGuard".into());
    let endpoint = req.endpoint_url.clone().unwrap_or_else(|| "memory://fake".into());
    let endpoint_classification = if req.provider == "fake" { "local".into() } else { classify_endpoint_url(&endpoint) };
    ...
    let source_agent_profile_id = req.agent_profile_id.clone().unwrap_or_else(|| "local-patcher".into());
    ...
    let prompt = format!(
        ...
        selected_content.chars().take(24000).collect::<String>(),
        req.selected_text.unwrap_or_default(),
        package_scripts,
        tree
    );
    let messages = vec![RuntimeMessage { role: "system".into(), content: system }, RuntimeMessage { role: "user".into(), content: prompt.clone() }];
    ...
    let included = vec![
        ContextIncludedView { kind: "selected_file".into(), path: Some(req.selected_file_path.clone()), chars: selected_content.len(), note: format!("beforeSha256={}", before_sha) },
        ContextIncludedView { kind: "file_tree_excerpt".into(), path: None, chars: tree.len(), note: format!("{} allowed entries, denied directories/files omitted", files.iter().filter(|f| !f.denied).count().min(120)) },
        ContextIncludedView { kind: "package_scripts".into(), path: Some("package.json".into()), chars: package_scripts.len(), note: "package metadata excerpt if available".into() },
    ];
    ...
    conn.execute(
        ...
        params![&context_bundle_id, &req.session_id, view.char_estimate as i64, &payload_json],
    ).map_err(|e| e.to_string())?;
    ...
    let _ = append_event(&conn, &req.session_id, "context.bundle_created", &payload.to_string());
    Ok(view)
}

#[tauri::command]
fn approve_runtime_endpoint(req: RuntimeEndpointApprovalRequest) -> Result<RuntimeEndpointApprovalResult, String> {
    let repo_root = PathBuf::from(&req.repo_root);
    let conn = init_audit(&repo_root, &req.session_id).map_err(|e| e.to_string())?;
    ...
    let approved_at = select_scalar_string(&conn, "SELECT approved_at FROM endpoint_approvals WHERE endpoint_url = ?1", &req.endpoint_url)?;
    ...
    let _ = append_event(&conn, &req.session_id, "runtime.endpoint_approved", &payload.to_string());
    Ok(RuntimeEndpointApprovalResult { endpoint_url: req.endpoint_url, endpoint_classification: classification, allow_repo_context: req.allow_repo_context, approved_at })
}

#[tauri::command]
fn runtime_endpoint_approval_status(req: RuntimeEndpointApprovalStatusRequest) -> Result<RuntimeEndpointApprovalStatusResult, String> {
    let classification = classify_endpoint_url(&req.endpoint_url);
    if classification == "local" {
        return Ok(RuntimeEndpointApprovalStatusResult { endpoint_url: req.endpoint_url, endpoint_classification: classification, approved: true, allow_repo_context: true, approved_at: None });
    }
    let repo_root = req.repo_root.clone().map(PathBuf::from).unwrap_or_else(|| std::env::temp_dir().join("synthesize-no-repo-approval-status"));
    let conn = init_audit(&repo_root, &req.session_id).map_err(|e| e.to_string())?;
    ...
    Ok(RuntimeEndpointApprovalStatusResult { endpoint_url: req.endpoint_url, endpoint_classification: classification, approved, allow_repo_context, approved_at })
}

#[tauri::command]
fn runtime_health_check(req: RuntimeHealthRequest) -> Result<RuntimeHealthResult, String> {
    let classification = if req.provider == "fake" { "local".into() } else { classify_endpoint_url(&req.endpoint_url) };
    ...
    if let Some(root) = repo_root.as_ref() {
        if let Ok(conn) = init_audit(root, &req.session_id) {
            ...
            let _ = append_event(&conn, &req.session_id, "runtime.health_check", &payload.to_string());
        }
    }
    match result {
        Ok(message) => Ok(RuntimeHealthResult { ok: true, provider: req.provider, endpoint_url: req.endpoint_url, endpoint_classification: classification, model: req.model, message }),
        Err(message) => Ok(RuntimeHealthResult { ok: false, provider: req.provider, endpoint_url: req.endpoint_url, endpoint_classification: classification, model: req.model, message }),
    }
}
...
fn list_runtime_models(req: RuntimeListModelsRequest) -> Result<Vec<RuntimeModelView>, String> {
    if req.provider == "fake" {
        return Ok(vec![RuntimeModelView { id: "fixture-patcher".into() }]);
    }
    let base = normalize_endpoint_base(&req.endpoint_url)?;
    let classification = classify_endpoint_url(&req.endpoint_url);
    let agent = ureq::AgentBuilder::new().timeout(std::time::Duration::from_secs(10)).build();
    let resp = agent.get(&format!("{}/models", base)).call().map_err(format_runtime_error)?;
    let json: serde_json::Value = resp.into_json().map_err(|e| e.to_string())?;
    let models = json.get("data").and_then(|v| v.as_array()).cloned().unwrap_or_default().into_iter().filter_map(|item| item.get("id").and_then(|id| id.as_str()).map(|s| RuntimeModelView { id: s.to_string() })).collect::<Vec<_>>();
    if let Some(root) = req.repo_root.as_ref().map(PathBuf::from) {
        if let Ok(conn) = init_audit(&root, &req.session_id) {
            ...
            let _ = append_event(&conn, &req.session_id, "runtime.models_listed", &payload.to_string());
        }
    }
    Ok(models)
}
...
fn runtime_generate(req: RuntimeGenerateRequest) -> Result<RuntimeGenerateResult, String> {
    ...
    if context.endpoint_classification != classification {
        return Err(format!("context bundle endpoint classification {} does not match runtime request {}", context.endpoint_classification, classification));
    }
    let messages = context.messages.clone();
    let messages_sha256 = hash_runtime_messages(&messages)?;
    if messages_sha256 != context.messages_sha256 {
        return Err("persisted context bundle messages hash mismatch; refusing runtime call".into());
    }
    ...
    let _ = append_event(&conn, &req.session_id, "runtime.request_started", &start_payload.to_string());
    let started = Instant::now();
    let generated = if req.provider == "fake" {
        Ok(fake_runtime_response(&messages))
    } else {
        call_openai_compatible_endpoint(&req.endpoint_url, &req.model, &messages, req.temperature, req.max_tokens, req.response_format.as_deref())
    };
    match generated {
        Ok(content) => {
            ...
            let event_id = append_event(&conn, &req.session_id, "runtime.request_completed", &payload.to_string()).map_err(|e| e.to_string())?;
            Ok(RuntimeGenerateResult { provider: req.provider, endpoint_url: req.endpoint_url, endpoint_classification: classification, model: req.model, content, duration_ms: duration, input_chars, output_chars, audit_event_id: Some(event_id) })
        }
        Err(err) => {
            let duration = started.elapsed().as_millis();
            ...
            let _ = append_event(&conn, &req.session_id, "runtime.request_failed", &payload.to_string());
            Err(err)
        }
    }
}
...
fn runtime_cancel(req: RuntimeCancelRequest) -> Result<RuntimeCancelResult, String> {
    let message = "runtime cancellation is not implemented for blocking backend calls in this build".to_string();
    Ok(RuntimeCancelResult { cancelled: false, message: format!("{}; session={}; request={}", message, req.session_id, req.request_id.unwrap_or_else(|| "none".into())) })
}
...
fn validate_patch_proposal(req: PatchProposalRequest) -> Result<PatchValidationResult, String> {
    ...
    let conn = init_audit(&repo_root, &req.session_id).map_err(|e| e.to_string())?;
    let (source_agent_profile_id, source_context_bundle_id) = source_profile_for_patch_validation(&conn, &req, &repo_root)?;
    ...
    if let Some(existing) = load_existing_proposal_identity(&conn, &proposal.id)? {
        if existing.operation_sha256 != operation_sha256 {
            return Err(format!("proposal_id {} already exists with a different operation_sha256", proposal.id));
        }
        ...
        if existing.status == "rejected" {
            return Ok(PatchValidationResult {
                ...
                files: load_patch_file_views(&conn, &proposal.id).unwrap_or_default(),
        ...
        if existing.status == "proposed" {
            return Err(format!("proposal {} is already proposed and awaiting validation transition", proposal.id));
        }
        if matches!(existing.status.as_str(), "approved" | "applying" | "applied" | "rolling_back" | "rolled_back" | "apply_failed" | "rollback_failed") {
            return Ok(PatchValidationResult {
                ...
                files: load_patch_file_views(&conn, &proposal.id).unwrap_or_default(),
                warnings: vec![format!("duplicate validation ignored; persisted proposal is already in {} state", existing.status)],
                errors: vec![],
    ...
    insert_patch_proposal(&conn, &req.session_id, &repo_root, &proposal, &operation_json, &operation_sha256, "proposed", None, Some(source_context_bundle_id.as_str()), &source_agent_profile_id)?;

    let actual_commit = git_current_commit(&repo_root);
    let validation = validate_patch_proposal_against_repo(&repo_root, &proposal, actual_commit.as_deref());

    match validation {
        Ok(files) => {
            persist_patch_files(&conn, &proposal, &files)?;
            transition_proposal_status(&conn, &req.session_id, &proposal.id, "proposed", "validated", None)?;
            let views = files.iter().map(|file| PatchFileValidationView {
                id: file.id.clone(),
            ...
            let audit_event_id = append_event(&conn, &req.session_id, "patch.validated", &payload.to_string()).map_err(|e| e.to_string())?;
        ...
        Err(err) => {
            ...
            transition_proposal_status(&conn, &req.session_id, &proposal.id, "proposed", "rejected", Some(&reason))?;
            ...
            let audit_event_id = append_event(&conn, &req.session_id, "patch.rejected", &payload.to_string()).map_err(|e| e.to_string())?;
            Ok(PatchValidationResult {
                ok: false,
                proposal_id: proposal.id,
                operation_sha256,
...
fn approve_patch_proposal(req: PatchApprovalRequest) -> Result<PatchApprovalView, String> {
    ...
    if stored.status != "validated" {
        return Err(format!("proposal {} must be validated before approval; current status is {}", req.proposal_id, stored.status));
    }
    ...
    transition_proposal_status(&tx, &req.session_id, &req.proposal_id, "validated", "approved", None)?;
    let approved_at = select_scalar_string(&tx, "SELECT approved_at FROM patch_approvals WHERE approval_id = ?1", &approval_id)?;
    ...
    let audit_event_id = append_event(&tx, &req.session_id, "patch.approved", &payload.to_string()).map_err(|e| e.to_string())?;
    tx.commit().map_err(|e| e.to_string())?;

    Ok(PatchApprovalView { proposal_id: req.proposal_id, approval_id, operation_sha256: req.operation_sha256, approved_by_source: "local-user".into(), approved_at, audit_event_id })
}
...
fn apply_approved_patch(req: PatchApplyRequest) -> Result<PatchApplyView, String> {
    ...
    if stored.status != "approved" {
        return Err(format!("proposal {} must be approved before apply; current status is {}", req.proposal_id, stored.status));
    }
    verify_approval(&conn, &req.proposal_id, &req.approval_id, &stored.operation_sha256)?;
    ...
    transition_proposal_status(&conn, &req.session_id, &req.proposal_id, "approved", "applying", None)?;
    let _ = append_event(&conn, &req.session_id, "patch.applying", &serde_json::json!({"proposal_id": &req.proposal_id, "approval_id": &req.approval_id}).to_string());

    let actual_commit = git_current_commit(&repo_root);
    let result = match apply_patch_proposal_transactional(&repo_root, &stored.proposal, actual_commit.as_deref()) {
        Ok(result) => result,
        Err(err) => {
            let reason = err.to_string();
            let _ = transition_proposal_status(&conn, &req.session_id, &req.proposal_id, "applying", "apply_failed", Some(&reason));
            let payload = serde_json::json!({ "proposal_id": &req.proposal_id, "approval_id": &req.approval_id, "error": &reason });
            let _ = append_event(&conn, &req.session_id, "patch.apply_failed", &payload.to_string());
            return Err(reason);
        }
    };

    let applied_files = result.applied_files.iter().map(|file| AppliedFileView { id: file.id.clone(), path: file.path.clone(), after_sha256: file.after_sha256.clone() }).collect::<Vec<_>>();
    ...
    let finalize_result: Result<String, String> = (|| {
        ...
        transition_proposal_status(&tx, &req.session_id, &req.proposal_id, "applying", "applied", None)?;
        ...
        let audit_event_id = append_event(&tx, &req.session_id, "patch.applied", &view_payload.to_string()).map_err(|e| e.to_string())?;
        tx.commit().map_err(|e| e.to_string())?;
        Ok(audit_event_id)
    })();
    ...
    let audit_event_id = match finalize_result {
        Ok(id) => id,
        Err(err) => {
            let restore = rollback_checkpoint_for_proposal(&repo_root, &result.checkpoint_id, &req.proposal_id);
            let reason = format!("post-write persistence/status/audit failed: {}; restore_result={:?}", err, restore.as_ref().err());
            let _ = transition_proposal_status(&conn, &req.session_id, &req.proposal_id, "applying", "apply_failed", Some(&reason));
...
fn rollback_patch(req: RollbackRequest) -> Result<RollbackView, String> {
    ...
    if stored.status != "applied" {
        return Err(format!("proposal {} must be applied before rollback; current status is {}", req.proposal_id, stored.status));
    }
    let checkpoint_id = stored.checkpoint_id.clone().ok_or_else(|| format!("proposal {} has no persisted checkpoint_id", req.proposal_id))?;
    ...
    let (checkpoint_repo, checkpoint_sha) = checkpoint_row.ok_or_else(|| "checkpoint record not found for applied proposal".to_string())?;
    ...
    transition_proposal_status(&conn, &req.session_id, &req.proposal_id, "applied", "rolling_back", None)?;
    let _ = append_event(&conn, &req.session_id, "patch.rolling_back", &serde_json::json!({"proposal_id": &req.proposal_id, "checkpoint_id": &checkpoint_id}).to_string());

    let result = match rollback_checkpoint_for_proposal(&repo_root, &checkpoint_id, &req.proposal_id) {
        Ok(result) => result,
        Err(err) => {
            let reason = err.to_string();
            let _ = transition_proposal_status(&conn, &req.session_id, &req.proposal_id, "rolling_back", "rollback_failed", Some(&reason));
            let payload = serde_json::json!({ "proposal_id": &req.proposal_id, "checkpoint_id": &checkpoint_id, "error": &reason });
            let _ = append_event(&conn, &req.session_id, "patch.rollback_failed", &payload.to_string());
            return Err(reason);
        }
    };
    ...
    let finalize_result: Result<String, String> = (|| {
        let tx = conn.transaction().map_err(|e| e.to_string())?;
        transition_proposal_status(&tx, &req.session_id, &req.proposal_id, "rolling_back", "rolled_back", None)?;
        ...
        let audit_event_id = append_event(&tx, &req.session_id, "patch.rolled_back", &payload.to_string()).map_err(|e| e.to_string())?;
        tx.commit().map_err(|e| e.to_string())?;
        Ok(audit_event_id)
    })();
    ...
    let audit_event_id = match finalize_result {
        Ok(id) => id,
        Err(err) => {
            let reason = format!("rollback restored files but status/audit finalization failed: {}", err);
            let _ = transition_proposal_status(&conn, &req.session_id, &req.proposal_id, "rolling_back", "rollback_failed", Some(&reason));
            let payload = serde_json::json!({ "proposal_id": &req.proposal_id, "checkpoint_id": &checkpoint_id, "error": &reason });
            let _ = append_event(&conn, &req.session_id, "patch.rollback_failed", &payload.to_string());
            return Err(reason);
...
fn read_guarded_file(repo_root: String, relative_path: String) -> Result<String, String> {
    let guard = RepoGuard::new(PathBuf::from(repo_root), FilePolicy::default()).map_err(|e| e.to_string())?;
    let path = guard.resolve_for_existing_path(relative_path).map_err(|e| e.to_string())?;
...
fn write_guarded_file(req: WriteFileRequest) -> Result<FileMutationResult, String> {
    let repo_root = PathBuf::from(&req.repo_root);
    let guard = RepoGuard::new(&repo_root, FilePolicy::default()).map_err(|e| e.to_string())?;
    let path = guard.resolve_for_write_path(&req.relative_path).map_err(|e| e.to_string())?;
    ...
    Ok(FileMutationResult { path: req.relative_path, message: "File saved through RepoGuard.".into(), audit_event_id })
}
...
fn create_repo_file(req: CreateFileRequest) -> Result<FileMutationResult, String> {
    ...
    let audit_event_id = init_audit(&repo_root, &req.session_id)
        .ok()
        .and_then(|conn| {
            let payload = serde_json::json!({ "path": req.relative_path, "operation": "create_repo_file" });
            append_event(&conn, &req.session_id, "file.created", &payload.to_string()).ok()
        });
    Ok(FileMutationResult { path: req.relative_path, message: "File created through RepoGuard.".into(), audit_event_id })
}
...
fn rename_repo_path(req: RenamePathRequest) -> Result<FileMutationResult, String> {
    let repo_root = PathBuf::from(&req.repo_root);
    let guard = RepoGuard::new(&repo_root, FilePolicy::default()).map_err(|e| e.to_string())?;
    let from = guard.resolve_for_existing_path(&req.from_path).map_err(|e| e.to_string())?;
    let to = guard.resolve_for_write_path(&req.to_path).map_err(|e| e.to_string())?;
    if to.exists() {
    ...
    Ok(FileMutationResult { path: req.to_path, message: "Path renamed through RepoGuard.".into(), audit_event_id })
}
...
fn reject_delete_attempt(repo_root: &Path, session_id: &str, relative_path: &str, reason: &str) {
    if let Ok(conn) = init_audit(repo_root, session_id) {
        ...
        let _ = append_event(&conn, session_id, "file.delete_rejected", &payload.to_string());
    }
}

fn is_dangerous_delete_path(relative_path: &str) -> bool {
    let trimmed = relative_path.trim();
    if trimmed.is_empty() || trimmed == "." || trimmed == "./" { return true; }
    let normalized = trimmed.replace('\\', "/").trim_matches('/').to_ascii_lowercase();
    normalized.is_empty()
        || normalized == "."
        || normalized == ".git"
...
fn delete_repo_path(req: DeletePathRequest) -> Result<FileMutationResult, String> {
    ...
    if is_dangerous_delete_path(&req.relative_path) {
        reject_delete_attempt(&repo_root, &req.session_id, &req.relative_path, "refusing to delete repo root or Synthesize/Git control directory");
        return Err("refusing to delete repo root or Synthesize/Git control directory".into());
    }
    let path = guard.resolve_for_existing_path(&req.relative_path).map_err(|e| e.to_string())?;
    let canonical_root = repo_root.canonicalize().map_err(|e| e.to_string())?;
    if path == canonical_root {
        reject_delete_attempt(&repo_root, &req.session_id, &req.relative_path, "refusing to delete repo root");
        return Err("refusing to delete repo root".into());
    }
    if path.is_dir() {
        if !req.allow_directory {
            reject_delete_attempt(&repo_root, &req.session_id, &req.relative_path, "directory delete requires allow_directory=true and confirmation token");
            return Err("directory delete requires explicit allow_directory=true and confirmation token".into());
        }
        let expected = format!("DELETE_DIRECTORY:{}", req.relative_path.trim());
        if req.confirmation_token.as_deref() != Some(expected.as_str()) {
            reject_delete_attempt(&repo_root, &req.session_id, &req.relative_path, "directory delete confirmation token mismatch");
            return Err(format!("directory delete requires confirmation token {}", expected));
        }
    ...
    Ok(FileMutationResult { path: req.relative_path, message: "Path deleted through RepoGuard with protected-delete checks.".into(), audit_event_id })
}
...
fn list_audit_events(repo_root: String, session_id: String) -> Result<Vec<AuditEventView>, String> {
    ...
    let rows = stmt.query_map([session_id], |row| Ok(AuditEventView { id: row.get(0)?, timestamp: row.get(1)?, kind: row.get(2)?, payload_json: row.get(3)? })).map_err(|e| e.to_string())?;
...
fn record_session_event(req: SessionEventRequest) -> Result<SessionEventResult, String> {
    let repo_root = PathBuf::from(&req.repo_root);
    let conn = init_audit(&repo_root, &req.session_id).map_err(|e| e.to_string())?;
    let id = append_event(&conn, &req.session_id, &req.kind, &req.payload_json).map_err(|e| e.to_string())?;
...
fn classify_command(req: CommandClassifyRequest) -> Result<CommandClassifyResult, String> {
    let guard_req = GuardCommandRequest { argv: req.argv.clone(), cwd: req.cwd.clone(), requires_network: req.requires_network, may_modify_files: req.may_modify_files };
    let result = match classify(&guard_req, &CommandPolicy::default()) {
        Ok(risk) => CommandClassifyResult { ok: true, risk: risk_label(&risk).into(), message: "command execution is disabled; this is classification only".into() },
        Err(err) => CommandClassifyResult { ok: false, risk: "Blocked".into(), message: err.to_string() },
    };
    if let (Some(repo_root), Some(session_id)) = (req.repo_root.as_ref(), req.session_id.as_ref()) {
        if let Ok(conn) = init_audit(Path::new(repo_root), session_id) {
            ...
            let _ = append_event(&conn, session_id, "command.classified", &payload.to_string());
        }
    }
    Ok(result)
}
...
fn project_search(req: ProjectSearchRequest) -> Result<Vec<ProjectSearchResult>, String> {
    ...
    for file in files.iter().filter(|f| f.kind == "file" && !f.denied && is_text_like(&f.path)) {
        if results.len() >= max { break; }
        let text = match read_guarded_file(req.repo_root.clone(), file.path.clone()) { Ok(t) => t, Err(_) => continue };
        for (idx, line) in text.lines().enumerate() {
            if line.to_ascii_lowercase().contains(&query.to_ascii_lowercase()) {
                results.push(ProjectSearchResult { path: file.path.clone(), line: idx + 1, preview: line.trim().chars().take(240).collect() });
                if results.len() >= max { break; }
            }
        }
    }
    let payload = serde_json::json!({ "query": query, "result_count": results.len(), "max_results": max });
    let _ = append_event(&conn, &req.session_id, "project.search", &payload.to_string());
    Ok(results)
}
...
fn git_status(req: GitStatusRequest) -> Result<GitStatusView, String> {
    ...
    let output = Command::new("git")
        .arg("-C").arg(&real_repo)
        .arg("status").arg("--short").arg("--branch")
        .output()
    ...
    for line in raw.lines() {
        if let Some(rest) = line.strip_prefix("## ") {
            ...
            if !path.is_empty() { files.push(GitStatusFileView { path, status }); }
        }
    }
...
fn git_diff_file(req: GitDiffRequest) -> Result<GitDiffView, String> {
    ...
    let _ = guard.resolve_for_existing_path(&req.path).map_err(|e| e.to_string())?;
    let mut cmd = Command::new("git");
    cmd.arg("diff");
    if req.staged { cmd.arg("--cached"); }
    cmd.arg("--").arg(&req.path).current_dir(&real_repo);
    let output = cmd.output().map_err(|e| format!("failed to run read-only git diff: {}", e))?;
    let raw = String::from_utf8_lossy(&output.stdout).to_string();
    ...
    Ok(GitDiffView { path: req.path, staged: req.staged, diff, truncated, audit_event_id })
}
...
fn git_stage_file(req: GitFileMutationRequest) -> Result<GitMutationResult, String> {
    ...
    let real_repo = repo_root.canonicalize().map_err(|e| e.to_string())?;
    let guard = RepoGuard::new(&real_repo, FilePolicy::default()).map_err(|e| e.to_string())?;
    let _ = guard.resolve_for_write_path(&req.path).map_err(|e| e.to_string())?;
    let output = Command::new("git").arg("add").arg("--").arg(&req.path).current_dir(&real_repo).output().map_err(|e| e.to_string())?;
    let stdout = String::from_utf8_lossy(&output.stdout).chars().take(8000).collect::<String>();
    let stderr = String::from_utf8_lossy(&output.stderr).chars().take(8000).collect::<String>();
    ...
    Ok(GitMutationResult { ok, message: if ok { "File staged.".into() } else { "git add failed.".into() }, stdout, stderr, audit_event_id })
}
...
fn git_unstage_file(req: GitFileMutationRequest) -> Result<GitMutationResult, String> {
    ...
    let guard = RepoGuard::new(&real_repo, FilePolicy::default()).map_err(|e| e.to_string())?;
    let _ = guard.resolve_for_write_path(&req.path).map_err(|e| e.to_string())?;
    let output = Command::new("git").arg("restore").arg("--staged").arg("--").arg(&req.path).current_dir(&real_repo).output().map_err(|e| e.to_string())?;
    let stdout = String::from_utf8_lossy(&output.stdout).chars().take(8000).collect::<String>();
    let stderr = String::from_utf8_lossy(&output.stderr).chars().take(8000).collect::<String>();
    let ok = output.status.success();
    let audit_event_id = init_audit(&real_repo, &req.session_id).ok().and_then(|conn| {
    ...
    Ok(GitMutationResult { ok, message: if ok { "File unstaged.".into() } else { "git restore --staged failed.".into() }, stdout, stderr, audit_event_id })
}
...
fn git_commit_changes(req: GitCommitRequest) -> Result<GitMutationResult, String> {
    ...
    if message.len() < 3 || message.len() > 500 { return Err("commit message must be 3..500 characters".into()); }
    let output = Command::new("git").arg("commit").arg("--no-verify").arg("-m").arg(message).current_dir(&real_repo).output().map_err(|e| e.to_string())?;
    let stdout = String::from_utf8_lossy(&output.stdout).chars().take(12000).collect::<String>();
    let stderr = String::from_utf8_lossy(&output.stderr).chars().take(12000).collect::<String>();
    let ok = output.status.success();
    let audit_event_id = init_audit(&real_repo, &req.session_id).ok().and_then(|conn| {
    ...
    Ok(GitMutationResult { ok, message: if ok { "Commit created with --no-verify.".into() } else { "git commit failed.".into() }, stdout, stderr, audit_event_id })
}
...
fn lsp_capabilities(req: LspCapabilityRequest) -> Result<Vec<LspCapabilityView>, String> {
    ...
    let has_ts = files.iter().any(|f| f.path.ends_with(".ts") || f.path.ends_with(".tsx") || f.path.ends_with("package.json") || f.path.ends_with("tsconfig.json"));
    let has_py = files.iter().any(|f| f.path.ends_with(".py") || f.path.ends_with("pyproject.toml"));
    let has_rs = files.iter().any(|f| f.path.ends_with(".rs") || f.path.ends_with("Cargo.toml"));
    let has_go = files.iter().any(|f| f.path.ends_with(".go") || f.path.ends_with("go.mod"));
    let caps = vec!["diagnostics", "hover", "go-to-definition", "find-references", "document-symbols", "formatting"].into_iter().map(|s| s.to_string()).collect::<Vec<_>>();
    let views = vec![
...
fn approve_personal_command(req: PersonalCommandRequest) -> Result<TaskApprovalView, String> {
    ...
    if req.argv.iter().any(|arg| arg.contains('\0') || arg.len() > 4096) {
    ...
    let guard = RepoGuard::new(&repo_root, FilePolicy::default()).map_err(|e| e.to_string())?;
    let _real_cwd = guard.resolve_for_existing_path(&req.cwd).map_err(|e| format!("command cwd must stay inside repo: {}", e))?;
    ...
    let conn = init_audit(&repo_root, &req.session_id).map_err(|e| e.to_string())?;
    if matches!(risk, CommandRisk::Network | CommandRisk::Destructive | CommandRisk::Blocked) {
        let payload = serde_json::json!({
            "argv": &req.argv,
            "cwd": &req.cwd,
            "risk": &risk_label,
        ...
        let _ = append_event(&conn, &req.session_id, "personal_command.approval_rejected", &payload.to_string());
    ...
    let _ = append_event(&conn, &req.session_id, "personal_command.approved", &payload.to_string());
    Ok(TaskApprovalView {
        command_id,
        task_id,
        risk: risk_label,
...
fn detect_tasks(req: TaskDetectRequest) -> Result<Vec<DetectedTaskView>, String> {
            ...
            if let Some(scripts) = json.get("scripts").and_then(|v| v.as_object()) {
                for key in ["test", "lint", "build", "typecheck"] {
                    if scripts.contains_key(key) {
                        let argv = if key == "test" { vec!["pnpm".into(), "test".into()] } else { vec!["pnpm".into(), "run".into(), key.into()] };
                        let guard_req = GuardCommandRequest { argv: argv.clone(), cwd: root_cwd.clone(), requires_network: false, may_modify_files: key == "build" };
                        let risk = classify(&guard_req, &CommandPolicy::default()).map(|r| risk_label(&r).to_string()).unwrap_or_else(|_| "Blocked".into());
    ...
    if guarded_manifest_exists(&repo_root, "Cargo.toml") {
        tasks.push(DetectedTaskView { id: "cargo-test".into(), label: "cargo test".into(), argv: vec!["cargo".into(), "test".into()], cwd: root_cwd.clone(), risk: "TestOrBuild".into(), reason: "Detected Cargo.toml".into(), requires_network: false, may_modify_files: false });
    }
    if guarded_manifest_exists(&repo_root, "pyproject.toml") || guarded_manifest_exists(&repo_root, "pytest.ini") {
        tasks.push(DetectedTaskView { id: "pytest".into(), label: "pytest".into(), argv: vec!["pytest".into()], cwd: root_cwd.clone(), risk: "TestOrBuild".into(), reason: "Detected Python test metadata".into(), requires_network: false, may_modify_files: false });
    }
    if guarded_manifest_exists(&repo_root, "go.mod") {
        tasks.push(DetectedTaskView { id: "go-test".into(), label: "go test ./...".into(), argv: vec!["go".into(), "test".into(), "./...".into()], cwd: root_cwd, risk: "TestOrBuild".into(), reason: "Detected go.mod".into(), requires_network: false, may_modify_files: false });
    }
    let canonical_repo = canonical_repo_root_string(&repo_root).map_err(|e| e.to_string())?;
    let conn = init_audit(&repo_root, &req.session_id).map_err(|e| e.to_string())?;
    let tx = conn.unchecked_transaction().map_err(|e| e.to_string())?;
    tx.execute("DELETE FROM task_snapshots WHERE session_id = ?1 AND repo_root = ?2", params![&req.session_id, &canonical_repo]).map_err(|e| e.to_string())?;
    for task in &tasks {
    ...
    let _ = append_event(&conn, &req.session_id, "tasks.detected", &payload.to_string());
    Ok(tasks)
}
...
fn approve_task(req: TaskApproveRequest) -> Result<TaskApprovalView, String> {
    ...
    if !repo_roots_equal(&row.5, &repo_root).map_err(|e| format!("task snapshot repo-root verification failed: {}", e))? {
    ...
    let guard_req = GuardCommandRequest { argv: argv.clone(), cwd: row.2.clone(), requires_network: row.6 != 0, may_modify_files: row.7 != 0 };
    let risk = classify(&guard_req, &CommandPolicy::default()).map_err(|e| e.to_string())?;
    let risk_label = risk_label(&risk).to_string();
    if risk_label != row.3 {
        return Err(format!("detected task risk changed from {} to {}; re-detect tasks before approving", row.3, risk_label));
    }
    if matches!(risk, CommandRisk::Network | CommandRisk::Destructive | CommandRisk::Blocked) {
        ...
        let _ = append_event(&conn, &req.session_id, "task.approval_rejected", &payload.to_string());
        return Ok(TaskApprovalView { command_id: "none".into(), task_id: req.task_id, risk: risk_label, approved: false, message: "Synthesize refuses network/destructive/blocked task execution.".into() });
    }
    let command_id = new_id("cmd");
    conn.execute(
        "INSERT INTO commands (id, session_id, task_id, repo_root, argv_json, cwd, risk, requires_network, may_modify_files, approved_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9, datetime('now'))",
    ...
    let _ = append_event(&conn, &req.session_id, "task.approved", &payload.to_string());
...
fn run_approved_task(req: TaskRunRequest) -> Result<TaskRunResult, String> {
    ...
    if row.7.is_none() { return Err("task has not been approved".into()); }
    if !repo_roots_equal(&row.1, &repo_root).map_err(|e| format!("approved task repo-root verification failed: {}", e))? {
    ...
    if argv.is_empty() { return Err("approved task has empty argv".into()); }
    let rerun_guard_req = GuardCommandRequest { argv: argv.clone(), cwd: row.3.clone(), requires_network: row.5 != 0, may_modify_files: row.6 != 0 };
    let rerun_policy = if row.0.starts_with("personal-") { personal_terminal_policy() } else { CommandPolicy::default() };
    ...
    if rerun_risk_label != row.4 {
        return Err(format!("approved task risk changed from {} to {}; re-detect and re-approve", row.4, rerun_risk_label));
    }
    if matches!(rerun_risk, CommandRisk::Network | CommandRisk::Destructive | CommandRisk::Blocked) {
        return Err("approved task is no longer executable under current command policy".into());
    }
    let guard = RepoGuard::new(&stored_repo_root, FilePolicy::default()).map_err(|e| e.to_string())?;
    let real_cwd = guard.resolve_for_existing_path(&row.3).map_err(|e| e.to_string())?;
    ...
    let _ = append_event(&conn, &req.session_id, "task.started", &start_payload.to_string());
    ...
    if let Some(stdout) = child.stdout.take() { spawn_bounded_log_reader(stdout, stdout_tail.clone()); }
    if let Some(stderr) = child.stderr.take() { spawn_bounded_log_reader(stderr, stderr_tail.clone()); }
    let started = Instant::now();
    ...
    let event_id = append_event(&conn, &req.session_id, if timed_out { "task.timed_out" } else { "task.finished" }, &payload.to_string()).ok();
...
fn runtime_status() -> RuntimeStatusView {
    RuntimeStatusView {
        ...
        notes: vec![
            "Fake runtime and local model server path are wired through backend-owned context/model calls.".into(),
            "Protocol label OpenAI-compatible means local HTTP wire protocol; Synthesize does not require an OpenAI account or cloud key.".into(),
...
fn list_curated_models() -> Vec<CuratedModelView> {
    vec![
        CuratedModelView { id: "qwen2.5-coder-7b-instruct-q4_k_m".into(), name: "Qwen2.5 Coder 7B Instruct Q4_K_M".into(), runtime: "llamacpp".into(), format: "gguf".into(), recommended_ram_gb: 12, notes: "Recommended GGUF class for local llama.cpp. Import a downloaded .gguf model or connect to a running local server.".into() },
        CuratedModelView { id: "qwen2.5-coder-14b-instruct-q4_k_m".into(), name: "Qwen2.5 Coder 14B Instruct Q4_K_M".into(), runtime: "llamacpp".into(), format: "gguf".into(), recommended_ram_gb: 20, notes: "Recommended GGUF class for local llama.cpp. Import a downloaded .gguf model or connect to a running local server.".into() },
    ]
}
...
fn list_runtime_presets() -> Vec<RuntimePresetView> {
    vec![
        RuntimePresetView { id: "llamacpp-server".into(), label: "llama.cpp server".into(), default_url: "http://localhost:8080/v1".into(), protocol: "OpenAI-compatible local HTTP".into(), notes: "Run a GGUF coding model locally with llama.cpp server.".into(), local_by_default: true },
        ...
        RuntimePresetView { id: "ollama-local".into(), label: "Ollama local".into(), default_url: "http://localhost:11434/v1".into(), protocol: "OpenAI-compatible local HTTP route".into(), notes: "Use Ollama's local OpenAI-compatible route when enabled/supported by your Ollama version.".into(), local_by_default: true },
        RuntimePresetView { id: "vllm-local".into(), label: "vLLM local server".into(), default_url: "http://localhost:8000/v1".into(), protocol: "OpenAI-compatible local HTTP".into(), notes: "For workstation/server GPU setups running open-source coding models.".into(), local_by_default: true },
...
fn import_local_model(req: ImportLocalModelRequest) -> Result<LocalModelView, String> {
    ...
    let sha = if req.calculate_sha256 { Some(sha256_file_for_model(&path)?) } else { None };
    let id = format!("gguf-{}", sha.as_deref().unwrap_or_else(|| path.file_stem().and_then(|s| s.to_str()).unwrap_or("model")));
    ...
    Ok(LocalModelView { id, display_name: req.display_name, local_path: path.to_string_lossy().to_string(), format: "gguf".into(), runtime_compatibility: "llamacpp".into(), size_bytes: Some(meta.len()), sha256: sha, imported_at: None })
}
...
fn list_local_models() -> Result<Vec<LocalModelView>, String> {
    ...
    let rows = stmt.query_map([], |row| Ok(LocalModelView { id: row.get(0)?, display_name: row.get(1)?, local_path: row.get(2)?, format: row.get(3)?, runtime_compatibility: row.get(4)?, size_bytes: row.get::<_, Option<i64>>(5)?.map(|v| v as u64), sha256: row.get(6)?, imported_at: row.get(7)? })).map_err(|e| e.to_string())?;
    let mut out = Vec::new();
...
#[tauri::command]
fn managed_llamacpp_validate_config(req: ManagedLlamaConfigRequest) -> Result<ManagedLlamaStatusView, String> {
    let binary = PathBuf::from(&req.binary_path);
    let model = PathBuf::from(&req.model_path);
    if !binary.exists() || !binary.is_file() { return Err("llama.cpp server binary path must point to an existing executable file".into()); }
    if !model.exists() || !model.is_file() { return Err("GGUF model path must point to an existing file".into()); }
    if model.extension().and_then(|s| s.to_str()).unwrap_or("").to_ascii_lowercase() != "gguf" { return Err("managed llama.cpp requires a .gguf model file".into()); }
...
#[tauri::command]
fn managed_llamacpp_start(req: ManagedLlamaConfigRequest) -> Result<ManagedLlamaStatusView, String> {
    managed_llamacpp_validate_config(ManagedLlamaConfigRequest { binary_path: req.binary_path.clone(), model_path: req.model_path.clone(), port: req.port, ctx_size: req.ctx_size })?;
    let port = req.port.unwrap_or(8080);
    ...
    let mut slot = mutex.lock().map_err(|_| "managed llama.cpp supervisor lock is poisoned".to_string())?;
    if let Some(existing) = slot.as_mut() {
        if let Some(status) = existing.child.try_wait().map_err(|e| e.to_string())? {
            ...
            *slot = None;
            return Ok(ManagedLlamaStatusView { status: "failed".into(), endpoint_url: Some(endpoint), pid: None, model_path: Some(req.model_path), binary_path: Some(req.binary_path), stdout_tail: Some(stdout_tail), stderr_tail: Some(stderr_tail), message: format!("previous managed llama.cpp process exited before start request; status={}", status) });
        }
        return Ok(ManagedLlamaStatusView { status: "ready-or-started".into(), endpoint_url: Some(existing.endpoint_url.clone()), pid: Some(existing.child.id()), model_path: Some(existing.model_path.clone()), binary_path: Some(existing.binary_path.clone()), stdout_tail: Some(tail_snapshot(&existing.stdout_tail)), stderr_tail: Some(tail_snapshot(&existing.stderr_tail)), message: "managed llama.cpp process is already tracked by Synthesize".into() });
    }
    let mut child = Command::new(&req.binary_path)
        .arg("--model").arg(&req.model_path)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .map_err(|e| format!("failed to start llama.cpp server with argv-only spawn: {}", e))?;
    let pid = child.id();
    let stdout_tail = Arc::new(Mutex::new(String::new()));
    let stderr_tail = Arc::new(Mutex::new(String::new()));
    if let Some(stdout) = child.stdout.take() { spawn_bounded_log_reader(stdout, stdout_tail.clone()); }
    if let Some(stderr) = child.stderr.take() { spawn_bounded_log_reader(stderr, stderr_tail.clone()); }
    *slot = Some(ManagedLlamaProcess { child, endpoint_url: endpoint.clone(), model_path: req.model_path.clone(), binary_path: req.binary_path.clone(), stdout_tail: stdout_tail.clone(), stderr_tail: stderr_tail.clone() });
    Ok(ManagedLlamaStatusView { status: "starting".into(), endpoint_url: Some(endpoint), pid: Some(pid), model_path: Some(req.model_path), binary_path: Some(req.binary_path), stdout_tail: Some(tail_snapshot(&stdout_tail)), stderr_tail: Some(tail_snapshot(&stderr_tail)), message: "managed llama.cpp server started; use health check to confirm readiness".into() })
}
...
fn managed_llamacpp_stop() -> Result<ManagedLlamaStatusView, String> {
    ...
    if let Some(mut existing) = slot.take() {
        ...
        Ok(ManagedLlamaStatusView { status: "stopped".into(), endpoint_url: Some(existing.endpoint_url), pid: None, model_path: Some(existing.model_path), binary_path: Some(existing.binary_path), stdout_tail: Some(stdout_tail), stderr_tail: Some(stderr_tail), message: "managed llama.cpp process stopped".into() })
    } else {
        Ok(ManagedLlamaStatusView { status: "stopped".into(), endpoint_url: None, pid: None, model_path: None, binary_path: None, stdout_tail: None, stderr_tail: None, message: "no managed llama.cpp process is currently tracked".into() })
    }
}
...
fn managed_llamacpp_status() -> ManagedLlamaStatusView {
    let mutex = MANAGED_LLAMA.get_or_init(|| Mutex::new(None));
    let mut slot = match mutex.lock() { Ok(s) => s, Err(_) => return ManagedLlamaStatusView { status: "failed".into(), endpoint_url: None, pid: None, model_path: None, binary_path: None, stdout_tail: None, stderr_tail: None, message: "managed llama.cpp supervisor lock is poisoned".into() } };
    if let Some(existing) = slot.as_mut() {
        match existing.child.try_wait() {
            Ok(Some(status)) => {
                ...
                *slot = None;
                ManagedLlamaStatusView { status: "failed".into(), endpoint_url: Some(endpoint), pid: None, model_path: Some(model), binary_path: Some(binary), stdout_tail: Some(stdout_tail), stderr_tail: Some(stderr_tail), message: format!("managed llama.cpp process exited; status={}", status) }
            }
            Ok(None) => ManagedLlamaStatusView { status: "started".into(), endpoint_url: Some(existing.endpoint_url.clone()), pid: Some(existing.child.id()), model_path: Some(existing.model_path.clone()), binary_path: Some(existing.binary_path.clone()), stdout_tail: Some(tail_snapshot(&existing.stdout_tail)), stderr_tail: Some(tail_snapshot(&existing.stderr_tail)), message: "managed llama.cpp process is tracked; use health check for readiness".into() },
            Err(err) => ManagedLlamaStatusView { status: "failed".into(), endpoint_url: Some(existing.endpoint_url.clone()), pid: Some(existing.child.id()), model_path: Some(existing.model_path.clone()), binary_path: Some(existing.binary_path.clone()), stdout_tail: Some(tail_snapshot(&existing.stdout_tail)), stderr_tail: Some(tail_snapshot(&existing.stderr_tail)), message: format!("failed to inspect managed llama.cpp process: {}", err) },
        }
    } else {
        ManagedLlamaStatusView { status: "stopped".into(), endpoint_url: None, pid: None, model_path: None, binary_path: None, stdout_tail: None, stderr_tail: None, message: "no managed llama.cpp process is currently tracked".into() }
    }
}
...
fn synthesize_app_data_dir() -> PathBuf {
    if let Ok(path) = std::env::var("SYNTHESIZE_APP_DATA_DIR") { return PathBuf::from(path); }
    #[cfg(target_os = "windows")]
    {
        if let Ok(appdata) = std::env::var("APPDATA") { return PathBuf::from(appdata).join("SynthesizeIDE"); }
    }
    #[cfg(target_os = "macos")]
    {
        if let Ok(home) = std::env::var("HOME") { return PathBuf::from(home).join("Library").join("Application Support").join("SynthesizeIDE"); }
    }
    if let Ok(xdg) = std::env::var("XDG_DATA_HOME") { return PathBuf::from(xdg).join("synthesize-ide"); }
    if let Ok(home) = std::env::var("HOME") { return PathBuf::from(home).join(".local").join("share").join("synthesize-ide"); }
...
fn build_synthesize_system_prompt(agent_profile_id: &str) -> String {
    let role_guidance = match agent_profile_id {
        ...
        "moa-action-planner" => "Profile: MoA Action Planner. Use a local model to create an explicit plan/action trace and emit typed operations for Synthesize/MoA governance. You may emit report, ask_user, propose_patch, final_report, and run_command suggestions. The model is not the actor: it proposes; Synthesize validates, approves, applies, rolls back, and audits.",
...
fn load_context_bundle(conn: &Connection, context_bundle_id: &str) -> Result<ContextBundleView, String> {
    let payload: String = conn.query_row(
        "SELECT payload_json FROM context_bundles WHERE id = ?1",
        params![context_bundle_id],
        |row| row.get(0),
    ).optional().map_err(|e| e.to_string())?.ok_or_else(|| format!("context bundle {} not found", context_bundle_id))?;
    serde_json::from_str(&payload).map_err(|e| format!("persisted context bundle is invalid JSON: {}", e))
}

fn hash_runtime_messages(messages: &[RuntimeMessage]) -> Result<String, String> {
    let canonical = canonical_json(&serde_json::to_value(messages).map_err(|e| e.to_string())?).map_err(|e| e.to_string())?;
...
fn verify_endpoint_approval(conn: &Connection, endpoint_url: &str, classification: &str) -> Result<(), String> {
    ...
    match allowed {
        Some(1) => Ok(()),
        _ => Err(format!("endpoint {} is {}; backend endpoint approval is required before sending repo context", sanitized_endpoint_for_log(endpoint_url), classification)),
    }
}
...
fn fake_runtime_response(messages: &[RuntimeMessage]) -> String {
    ...
    let before_sha = extract_prompt_value(&prompt, "beforeSha256").unwrap_or_else(|| "fixture-before-sha256".into());
    let current_file = extract_prompt_value(&prompt, "currentFile").unwrap_or_else(|| "src/auth/refresh.ts".into());
...
fn extract_prompt_value(prompt: &str, key: &str) -> Option<String> {
    ...
    prompt.lines().find_map(|line| line.strip_prefix(&needle).map(|v| v.trim().to_string()))
}

fn local_chat_request_body(model: &str, messages: &[RuntimeMessage], temperature: f32, max_tokens: u32, response_format: Option<&str>) -> Result<serde_json::Value, String> {
...
fn call_openai_compatible_endpoint(endpoint_url: &str, model: &str, messages: &[RuntimeMessage], temperature: f32, max_tokens: u32, response_format: Option<&str>) -> Result<String, String> {
    ...
    let resp = agent.post(&format!("{}/chat/completions", base))
        .set("Content-Type", "application/json")
        .send_json(body)
        .map_err(format_runtime_error)?;
    let json: serde_json::Value = resp.into_json().map_err(|e| format!("endpoint returned invalid JSON: {}", e))?;
    let content = json.get("choices")
        ...
        .and_then(|v| v.as_str())
        .ok_or_else(|| "endpoint response did not include choices[0].message.content".to_string())?;
...
fn format_runtime_error(err: ureq::Error) -> String {
    match err {
        ureq::Error::Status(code, resp) => {
            ...
            format!("endpoint returned HTTP {}: {}", code, text.chars().take(800).collect::<String>())
        }
...
fn enforce_agent_profile_allows_operation(agent_profile_id: &str, operation: &AgentOperation) -> Result<(), String> {
    match operation {
...
fn source_profile_for_patch_validation(
    ...
    Ok((context.agent_profile_id, req.context_bundle_id.clone()))
}

fn proposal_from_operation(operation: &AgentOperation) -> Result<PatchProposal, String> {
    match operation {
        AgentOperation::ProposePatch { proposal_id, current_commit, base_commit, files, .. } => Ok(PatchProposal {
...
fn protocol_file_to_engine_file(file: &ProtocolPatchFile) -> PatchFile {
    PatchFile { id: file.id.clone(), path: file.path.clone(), before_sha256: file.before_sha256.clone(), unified_diff: file.patch.clone() }
}

fn init_audit(repo_root: &Path, session_id: &str) -> Result<Connection, Box<dyn std::error::Error>> {
    fs::create_dir_all(repo_root.join(".synthesize"))?;
    let conn = Connection::open(repo_root.join(".synthesize").join("synthesize-audit.sqlite"))?;
    migrate_legacy_patch_tables(&conn)?;
    init_schema(&conn)?;
    ensure_task_schema(&conn)?;
    let canonical_repo = canonical_repo_root_string(repo_root).unwrap_or_else(|_| repo_root.to_string_lossy().to_string());
    conn.execute("INSERT OR IGNORE INTO sessions (id, repo_root, git_commit_start) VALUES (?1, ?2, ?3)", (session_id, canonical_repo, git_current_commit(repo_root)))?;
    Ok(conn)
}
...
fn ensure_task_schema(conn: &Connection) -> Result<(), rusqlite::Error> {
    ...
    if !command_cols.iter().any(|c| c == "requires_network") {
        let _ = conn.execute("ALTER TABLE commands ADD COLUMN requires_network INTEGER NOT NULL DEFAULT 0", []);
    }
    if !command_cols.iter().any(|c| c == "may_modify_files") {
        let _ = conn.execute("ALTER TABLE commands ADD COLUMN may_modify_files INTEGER NOT NULL DEFAULT 0", []);
    }
    Ok(())
}
...
fn migrate_legacy_patch_tables(conn: &Connection) -> Result<(), rusqlite::Error> {
    if table_exists(conn, "patch_proposals")? {
        ...
        let sql: String = conn.query_row("SELECT sql FROM sqlite_master WHERE type='table' AND name='patch_proposals'", [], |row| row.get(0))?;
            ...
            || !sql.contains("rollback_failed")
        {
            ...
            conn.execute(&format!("ALTER TABLE patch_proposals RENAME TO {}", legacy), [])?;
        }
    }
...
fn ensure_task_snapshots_repo_scoped_pk(conn: &Connection) -> Result<(), rusqlite::Error> {
    ...
    if pk_cols == vec!["task_id".to_string(), "session_id".to_string(), "repo_root".to_string()] {
        return Ok(());
    }
    conn.execute_batch(
...
fn table_exists(conn: &Connection, table: &str) -> Result<bool, rusqlite::Error> {
    let count: i64 = conn.query_row("SELECT count(*) FROM sqlite_master WHERE type='table' AND name=?1", params![table], |row| row.get(0))?;
    Ok(count > 0)
}
...
fn load_existing_proposal_identity(conn: &Connection, proposal_id: &str) -> Result<Option<ExistingProposalIdentity>, String> {
    conn.query_row(
        "SELECT operation_sha256, status FROM patch_proposals WHERE proposal_id = ?1",
        params![proposal_id],
        |row| Ok(ExistingProposalIdentity { operation_sha256: row.get(0)?, status: row.get(1)? }),
    ).optional().map_err(|e| e.to_string())
}

fn ensure_duplicate_is_same_hash(conn: &Connection, proposal_id: &str, operation_sha256: &str) -> Result<(), String> {
    if let Some(existing) = load_existing_proposal_identity(conn, proposal_id)? {
        if existing.operation_sha256 != operation_sha256 {
            return Err(format!("proposal_id {} already exists with a different operation_sha256", proposal_id));
        }
    }
    Ok(())
}
...
fn persist_patch_files(conn: &Connection, proposal: &PatchProposal, files: &[patch_engine::ValidatedPatchFile]) -> Result<(), String> {
    for file in &proposal.files {
        let risk = files.iter().find(|v| v.id == file.id).map(|v| format!("{:?}", v.risk)).unwrap_or_else(|| "Unknown".into());
        conn.execute(
...
fn load_patch_file_views(conn: &Connection, proposal_id: &str) -> Result<Vec<PatchFileValidationView>, String> {
    let mut stmt = conn.prepare("SELECT file_id, path, risk FROM patch_files WHERE proposal_id = ?1 ORDER BY rowid ASC").map_err(|e| e.to_string())?;
    let rows = stmt.query_map(params![proposal_id], |row| Ok(PatchFileValidationView {
        id: row.get(0)?,
        path: row.get(1)?,
        risk: row.get(2)?,
    ...
    let mut files = Vec::new();
    for row in rows { files.push(row.map_err(|e| e.to_string())?); }
    Ok(files)
}
...
fn load_stored_proposal(conn: &Connection, proposal_id: &str) -> Result<StoredProposal, String> {
    ...
    let rows = stmt.query_map(params![proposal_id], |row| Ok(PatchFile { id: row.get(0)?, path: row.get(1)?, before_sha256: row.get(2)?, unified_diff: row.get(3)? })).map_err(|e| e.to_string())?;
    let mut files = Vec::new();
    for row in rows { files.push(row.map_err(|e| e.to_string())?); }
    if files.is_empty() {
        return Err(format!("proposal {} has no persisted patch files", proposal_id));
    }
    Ok(StoredProposal { proposal: PatchProposal { id: proposal_id.to_string(), base_commit: None, current_commit, files }, operation_sha256, operation_json, status, checkpoint_id, checkpoint_dir })
}

fn transition_proposal_status(conn: &Connection, session_id: &str, proposal_id: &str, expected_from: &str, to: &str, reason: Option<&str>) -> Result<(), String> {
    if !is_legal_transition(expected_from, to) {
        return Err(format!("illegal lifecycle transition {} -> {}", expected_from, to));
    }
    let current: Option<String> = conn.query_row("SELECT status FROM patch_proposals WHERE proposal_id = ?1", params![proposal_id], |row| row.get(0)).optional().map_err(|e| e.to_string())?;
    let current = current.ok_or_else(|| format!("proposal {} not found", proposal_id))?;
    if current != expected_from {
        return Err(format!("proposal {} expected state {}, found {}", proposal_id, expected_from, current));
    }
    ...
    let changed = conn.execute(&sql, params![proposal_id, to, reason, expected_from]).map_err(|e| e.to_string())?;
    if changed != 1 {
        return Err(format!("lifecycle transition {} -> {} failed due to concurrent state change", expected_from, to));
    }
    let payload = serde_json::json!({ "proposal_id": proposal_id, "from": expected_from, "to": to, "reason": reason });
    let _ = append_event(conn, session_id, "patch.lifecycle_transition", &payload.to_string());
    Ok(())
}
...
fn verify_approval(conn: &Connection, proposal_id: &str, approval_id: &str, operation_sha256: &str) -> Result<(), String> {
...
struct RepoMutationLockGuard { key: String }
...
fn acquire_repo_mutation_lock(repo_root: &Path, session_id: &str, operation: &str) -> Result<RepoMutationLockGuard, String> {
    let key = repo_root.canonicalize().unwrap_or_else(|_| repo_root.to_path_buf()).to_string_lossy().to_string();
    let mutex = REPO_MUTATION_LOCKS.get_or_init(|| Mutex::new(HashSet::new()));
    let mut set = mutex.lock().map_err(|_| "repo mutation lock is poisoned".to_string())?;
    if !set.insert(key.clone()) {
        if let Ok(conn) = init_audit(repo_root, session_id) {
            let payload = serde_json::json!({ "repo_root": key, "operation": operation, "error": "repo mutation already in progress" });
            let _ = append_event(&conn, session_id, "patch.mutation_lock_conflict", &payload.to_string());
        }
        return Err(format!("another apply/rollback mutation is already running for repo {}", repo_root.to_string_lossy()));
    }
...
fn canonical_repo_root_string(repo_root: &Path) -> Result<String, String> {
    repo_root.canonicalize().map(|p| p.to_string_lossy().to_string()).map_err(|e| e.to_string())
}
...
fn repo_roots_equal(stored_repo_root: &str, requested_repo_root: &Path) -> Result<bool, String> {
    ...
    let requested_canonical = requested_repo_root.canonicalize().map_err(|e| e.to_string())?;
    Ok(stored_canonical == requested_canonical)
}

fn select_scalar_string(conn: &Connection, sql: &str, arg: &str) -> Result<String, String> {
    conn.query_row(sql, params![arg], |row| row.get(0)).map_err(|e| e.to_string())
}

fn git_current_commit(repo_root: &Path) -> Option<String> {
    let output = Command::new("git").arg("rev-parse").arg("HEAD").current_dir(repo_root).output().ok()?;
...
fn walk_repo(root: &Path, dir: &Path, guard: &RepoGuard, out: &mut Vec<RepoFileView>, limit: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for entry in fs::read_dir(dir)? {
        ...
        if file_type.is_symlink() {
            ...
            out.push(RepoFileView { path: rel, kind: "symlink-skipped".into(), denied: true });
            continue;
        }
        if is_ignored_repo_dir_name(name) { continue; }
        let rel = path.strip_prefix(root)?.to_string_lossy().replace('\\', "/");
        if file_type.is_dir() {
            if is_denied_context_dir_name(name) || guard.resolve_for_write_path(Path::new(&rel).join("__synthesize_policy_probe__")).is_err() {
                // Include only the denied directory itself. Do not descend or reveal child names.
                out.push(RepoFileView { path: format!("{}/", rel.trim_end_matches('/')), kind: "dir-denied".into(), denied: true });
                continue;
            }
            out.push(RepoFileView { path: rel.clone(), kind: "dir".into(), denied: false });
        ...
        if denied {
            ...
            out.push(RepoFileView { path: rel, kind: "file-denied".into(), denied: true });
        } else {
            out.push(RepoFileView { path: rel, kind: "file".into(), denied: false });
        }
    }
    Ok(())
}
...
fn is_ignored_repo_dir_name(name: &str) -> bool {
    matches!(name, "node_modules" | "target" | ".git" | "dist" | "build" | ".next" | ".synthesize")
}

fn is_denied_context_dir_name(name: &str) -> bool {
    let lower = name.to_ascii_lowercase();
    lower.starts_with('.')
        || matches!(lower.as_str(), "credentials" | "credential" | "secrets" | "secret" | "private" | "keys" | "key" | "certs" | "certificates")
...
fn is_text_like(path: &str) -> bool {
    ...
    [".ts", ".tsx", ".js", ".jsx", ".rs", ".py", ".go", ".java", ".md", ".json", ".toml", ".yaml", ".yml", ".css", ".html"].iter().any(|ext| lower.ends_with(ext))
}
...
mod tests {
    ...
    fn memory_conn() -> Connection {
        ...
        conn.execute("INSERT INTO sessions (id, repo_root) VALUES ('s', '/tmp/repo')", []).unwrap();
        conn
    }
    ...
    fn transition_helper_prevents_double_apply_state() {
        ...
        assert!(transition_proposal_status(&conn, "s", "p1", "approved", "applying", None).is_err());
    }
    ...
    fn lifecycle_transition_is_not_blocked_by_lifecycle_audit_failure() {
        ...
        let result = transition_proposal_status(&conn, "s", "p-phase", "approved", "applying", None);
        assert!(result.is_ok());
        let status: String = conn.query_row("SELECT status FROM patch_proposals WHERE proposal_id = 'p-phase'", [], |row| row.get(0)).unwrap();
        assert_eq!(status, "applying");
    }

    #[test]
    fn repo_roots_equal_canonicalizes_equivalent_paths() {
        let root = std::env::temp_dir().join(format!("synthesize-root-canon-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn denied_context_directory_child_names_are_not_listed() {
        let root = std::env::temp_dir().join(format!("synthesize-denied-dir-no-child-names-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        ...
        assert!(files.iter().any(|f| f.path == ".secrets/" && f.kind == "dir-denied" && f.denied));
    ...
    fn endpoint_classification_distinguishes_local_private_lan_and_remote() {
        ...
        assert_eq!(classify_endpoint_url("http://192.168.1.55:8080/v1"), "private-lan");
    ...
    fn fake_runtime_response_returns_typed_patch_operation() {
        let messages = vec![RuntimeMessage { role: "user".into(), content: "currentFile=src/auth/refresh.ts\nbeforeSha256=abc123\ncurrentCommit=deadbeef".into() }];
        let raw = fake_runtime_response(&messages);
        let parsed: serde_json::Value = serde_json::from_str(&raw).unwrap();
        assert_eq!(parsed["operations"][0]["type"], "propose_patch");
        assert_eq!(parsed["operations"][0]["files"][0]["beforeSha256"], "abc123");
    }
    ...
    fn runtime_messages_are_derived_from_persisted_context_bundle() {
        let conn = memory_conn();
        let messages = vec![RuntimeMessage { role: "system".into(), content: "s".into() }, RuntimeMessage { role: "user".into(), content: "u".into() }];
        let view = ContextBundleView {
        ...
        assert_eq!(loaded.messages_sha256, hash_runtime_messages(&loaded.messages).unwrap());
    }

    #[test]
    fn symlink_file_tree_entry_is_skipped_not_traversed() {
        let root = std::env::temp_dir().join(format!("synthesize-symlink-skip-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(root.join("src")).unwrap();
        std::fs::write(root.join("src").join("a.ts"), "export const a = 1;\n").unwrap();
        #[cfg(unix)]
        {
            ...
            let files = list_files_internal(&root, 20).unwrap();
            assert!(files.iter().any(|f| f.path == "linked-tmp" && f.kind == "symlink-skipped" && f.denied));
            assert!(!files.iter().any(|f| f.path.starts_with("linked-tmp/")));
        }
    }

    fn load_stored_proposal_allow_empty_for_test(conn: &Connection, proposal_id: &str) -> Result<StoredProposal, String> {
        ...
        Ok(StoredProposal { proposal: PatchProposal { id: proposal_id.to_string(), base_commit: None, current_commit: None, files: vec![PatchFile { id: "f".into(), path: "src/a.ts".into(), before_sha256: "x".into(), unified_diff: "diff".into() }] }, operation_sha256, operation_json, status, checkpoint_id: None, checkpoint_dir: None })
    }


    #[test]
    fn runtime_presets_are_local_first_and_no_cloud_presets() {
        let presets = list_runtime_presets();
        assert!(presets.iter().any(|p| p.id == "llamacpp-server"));
        assert!(presets.iter().any(|p| p.id == "lm-studio"));
        assert!(presets.iter().all(|p| p.local_by_default));
        assert!(presets.iter().all(|p| !p.label.to_ascii_lowercase().contains("openai cloud")));
    }

    #[test]
    fn managed_llamacpp_rejects_missing_binary_or_model() {
        let req = ManagedLlamaConfigRequest { binary_path: "/definitely/missing/llama-server".into(), model_path: "/definitely/missing/model.gguf".into(), port: Some(8080), ctx_size: Some(8192) };
    ...
    fn agent_profile_policy_allows_only_patch_capable_profiles_for_patch_validation() {
        let op = AgentOperation::ProposePatch {
            ...
            base_commit: None,
            current_commit: None,
            files: vec![agent_protocol::PatchFile { id: "f1".into(), path: "src/a.ts".into(), before_sha256: "abc".into(), patch: r#"diff --git a/src/a.ts b/src/a.ts
--- a/src/a.ts
+++ b/src/a.ts
@@ -1 +1 @@
-a
...
"#.into() }],
    ...
    fn source_profile_policy_uses_persisted_context_bundle() {
        let conn = Connection::open_in_memory().unwrap();
        init_schema(&conn).unwrap();
        let tmp = std::env::temp_dir().join(format!("synthesize-source-profile-{}", new_id("test")));
        ...
        let context = ContextBundleView {
            ...
            char_estimate: 1,
            included: vec![],
            messages: vec![RuntimeMessage { role: "user".into(), content: "x".into() }],
            exact_prompt: "x".into(),
    ...
    fn patch_validation_requires_context_bundle_id() {
        let conn = Connection::open_in_memory().unwrap();
        init_schema(&conn).unwrap();
        let tmp = std::env::temp_dir().join(format!("synthesize-missing-context-{}", new_id("test")));
        std::fs::create_dir_all(&tmp).unwrap();
        let op = AgentOperation::ProposePatch {
            ...
            base_commit: None,
            current_commit: None,
            files: vec![ProtocolPatchFile { id: "f".into(), path: "src/lib.ts".into(), before_sha256: "abc".into(), patch: r#"diff --git a/src/lib.ts b/src/lib.ts
--- a/src/lib.ts
+++ b/src/lib.ts
@@ -1 +1 @@
-a
...
"#.into() }],
    ...
    fn task_snapshot_approval_requires_detected_task() {
        ...
        let err = approve_task(TaskApproveRequest { session_id: "s".into(), repo_root: dir.to_string_lossy().to_string(), task_id: "unknown".into() }).unwrap_err();
    ...
    fn report_only_command_status_text_is_not_stale() {
        ...
        assert!(status.notes.iter().any(|n| n.contains("Agent-suggested commands remain classification-only")));
        assert!(status.notes.iter().any(|n| n.contains("Backend-detected tasks can run")));
    }
    ...
    fn task_snapshot_repo_root_must_match_approval_repo() {
        ...
        let row_repo: String = conn.query_row("SELECT repo_root FROM task_snapshots WHERE task_id = 'task-a'", [], |row| row.get(0)).unwrap();
    ...
    fn approved_task_repo_root_must_match_run_repo() {
        ...
        let row_repo: String = conn.query_row("SELECT repo_root FROM commands WHERE id = 'cmd-a'", [], |row| row.get(0)).unwrap();
        assert!(!repo_roots_equal(&row_repo, &repo_b).unwrap());
    }
